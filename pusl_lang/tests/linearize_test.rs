...
use pusl_lang::backend::linearize::{linearize_file, ByteCodeFile, BasicFunction, Function, ByteCode, ByteCodeArray};
...
fn check_code_equal(expect: &ByteCodeArray, actual: &ByteCodeArray, context: &str) {
    let expect_int = expect.0.iter().map(|bc| unsafe { bc.to_u64()} ).collect::<Vec<_>>();
    let actual_int = actual.0.iter().map(|bc| unsafe { bc.to_u64()} ).collect::<Vec<_>>();
...
fn check_function_equal(expect: &Function, actual: &Function, context: &str) {
    let Function { args: ex_args, binds: ex_binds, literals: ex_literals, references: ex_references, code: ex_code, is_generator: ex_is_generator  } = expect;
    let Function { args: ac_args, binds: ac_binds, literals: ac_literals, references: ac_references, code: ac_code, is_generator: ac_is_generator  } = actual;

    assert_eq!(ex_args, ac_args, "Function {} arguments mismatch", context);
    assert_eq!(ex_binds, ac_binds, "Function {} binds mismatch", context);
    assert_eq!(ex_literals, ac_literals, "Function {} literals mismatch", context);
    assert_eq!(ex_references, ac_references, "Function {} references mismatch", context);
    check_code_equal(ex_code, ac_code, context);
    assert_eq!(ex_is_generator, ac_is_generator, "Function {} is_generator mismatch", context);
}

fn check_basic_function_equal(expect: &BasicFunction, actual: &BasicFunction, context: &str) {
    let BasicFunction { function: ex_function, sub_functions: ex_sub_functions  } = expect;
    let BasicFunction { function: ac_function, sub_functions: ac_sub_functions  } = actual;

    check_function_equal(ex_function, ac_function, context);

    for (idx, (esf, asf)) in ex_sub_functions.into_iter().zip(ac_sub_functions).enumerate() {
...
fn check_bcf_equal(expect: &ByteCodeFile, actual: &ByteCodeFile) {
    let ByteCodeFile { file: ex_file, base_func: ex_base_func, imports: ex_imports  } = expect;
    let ByteCodeFile { file: ac_file, base_func: ac_base_func, imports: ac_imports  } = actual;
    ...
    assert_eq!(ex_imports.len(), ac_imports.len(), "Imports length mismatch");
