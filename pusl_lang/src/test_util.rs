    ...
    F: FnOnce(&T, &T),
{
    ...
    let actual_json =
        serde_json::to_string_pretty(&actual).expect("Cannot serialize actual output json");
    let mut out_enc = XzEncoder::new(
        fs::File::create(actual_file).expect("Cannot open actual output json"),
        9,
    );
    out_enc
        .write_all(actual_json.as_bytes())
        .expect("Cannot write actual output json");

    let mut in_dec =
        XzDecoder::new(fs::File::open(expect_file).expect("Cannot find expected output json"));
    let mut expect_json = String::new();
    in_dec
        .read_to_string(&mut expect_json)
        .expect("Cannot find expected output json");
    let expected =
        serde_json::from_str::<T>(&expect_json).expect("Cannot parse expected output json");
